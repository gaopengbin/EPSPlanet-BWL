<template>
  <div :id="container" style="width: 100%; height: 100%; display: inline-block; position: relative">
    <slot />
  </div>
</template>
...
defineOptions({ name: 'ep-earth' });

const props = defineProps(earthProps)
const emit = defineEmits(earthEmits)
/* eslint-disable */
// console.log(earthProps);
// const container = earthProps.container;
console.log(props);


// const params = reactive(props.params);
const container = props.container;
// const showCompass = params?.showCompass ?? reactive(props.showCompass);
// const showDistanceLegend =
//   params?.showDistanceLegend ?? reactive(props.showDistanceLegend);
// const compassStyle = params?.compassStyle ?? reactive(props.compassStyle);
...
// const showDefaultBasemap =
//   params?.showDefaultBasemap ?? reactive(props.showDefaultBasemap);
// const distanceLegendStyle =
//   params?.distanceLegendStyle ?? reactive(props.distanceLegendStyle);
let _earth: any;
// const data = reactive({
//   params,
//   container,
...
function init() {
  XE.ready()
    ...
    .then(() => {
        ...
        list.forEach((item: { children: any[]; title: string | number; }) => {
          if (item.children) {
            ...
            item.children.forEach((e: string | number) => {
              earth.sceneTree[item.title][e] = new XE.SceneTree.Group(
                earth
              );
      ...
      //                 url:
      //                   XE.HTML.cesiumDir + "Assets/Textures/NaturalEarthII",
      //                 fileExtension: "jpg",
      //               },
      ...
      //       earth.analyzation.measurement.clearGroundDistanceFunctions.forEach((item: () => void) => {
      //         item();
      //       })
      //     }
      //     earth.analyzation.measurement.clearThreeDAreaFunctions = [];
      //     earth.analyzation.measurement.clearThreeDArea = function () {
      //       earth.analyzation.measurement.clearThreeDAreaFunctions.forEach((item: () => void) => {
      //         item();
      //       })
      //     }
