...
export class PickModel {
    ...
    public pick(callback) {
        ...
        this.handler.setInputAction(async (click) => {
            ...
            do {
                ...
                i++
                if (!Cesium.defined(obj) || !obj.getPropertyIds || !allowPicking) {
                    // this.earth.sceneTree.$refs.pin1.czmObject.customProp = false;
                    callback(null)
                } else {
                    // let times = 0
                    ...
                    if (layer) {
                        ...
                        if (obj.getPropertyIds().length <= 2 && obj.getProperty('name') && obj.getProperty('name').indexOf('{') > -1) {
                    ...
                    function simplePick() {
                        let PropertyNames = obj.getPropertyIds()
    ...
    clearHighLight() {
        if (this.pickedTileset) {
            // this.pickedTileset.xbsjStyle = undefined
            this.pickedTileset.color = Cesium.Color.WHITE.withAlpha(1)
        }
    }
    // 记录当前鼠标位置经纬度
