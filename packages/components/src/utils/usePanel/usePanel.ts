...
export function usePanel(options: any, appContext?: AppContext | null, slot?: VNode | String) {

    // console.log(options, appContext, slot)
    if ((options.onlyOne && options.type == 'panel') || options.type == 'switch') {
    ...
    if (isVNode(slot)) {
        ...
        (slot as any).type.components = Object.assign((slot as any).type.components, appContext?.app._context.components);
        //把参数传给内容组件
        if (options.param) {
            slot.props = {
                ...options.param
