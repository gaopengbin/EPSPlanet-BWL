...
const groundImageData = ref<any>([]);
const groundRoadData = ref<any>([]);
...
onMounted(() => {
  ...
  disposer.push(
    ...
    XE.MVVM.watch(
      () => {
        if (earth.sceneTree) {
          return earth.sceneTree.plots.groundImage.toJSONStr();
        }
      },
      () => {
        getGroundImageList();
    ...
    XE.MVVM.watch(
      () => {
        if (earth.sceneTree) {
          return earth.sceneTree.plots.groundRoad.toJSONStr();
        }
      },
      () => {
        getGroundRoadList();
...
function getPolylineList() {
  let list: any = [];
  earth.sceneTree.plots.polyline.children.forEach((p: any) => {
    let json = p.toJSON();
    if (p.czmObject) {
...
function getGroundImageList() {
  let list = earth.sceneTree.plots.groundImage.toJSON().children;
  groundImageData.value = list;
}
function getGroundRoadList() {
  let list = earth.sceneTree.plots.groundRoad.toJSON().children;
  groundRoadData.value = list;
}
// function getFlatten() {
...
function refresh() {
  ...
  switch (props.type) {
    ...
    case 'particle':
      ...
      break;
    case 'groundImage':
      tableData.value = groundImageData.value;
      break;
    case 'groundRoad':
      tableData.value = groundRoadData.value;
      break;
    // case "flatten":
